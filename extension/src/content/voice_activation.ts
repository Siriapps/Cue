...
// Listen for toggle events from halo.tsx settings panel
if (typeof window !== "undefined") {
  window.addEventListener("cue:start-voice-activation", () => {
    startVoiceActivation().catch(() => { /* ignore */ });
  });
  window.addEventListener("cue:stop-voice-activation", () => {
    stopVoiceActivation();
  });
}
