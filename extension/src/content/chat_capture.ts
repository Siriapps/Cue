...
function extractGenericChatMessages(): CapturedMessage[] {
  // Heuristic selectors that often represent message-level containers.
  const selector = [
...
function computeHash(msgs: CapturedMessage[]): string {
  // Cheap hash to avoid spamming background.
  return msgs
    .slice(0, 50)
    .map((m) => `${m.role}:${m.text}`)
    .join("|")
...
export function initChatCapture(): void {
  ...
  // Observe changes to chat transcript.
